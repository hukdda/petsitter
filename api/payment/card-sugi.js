...
export default async function handler(req, res) {
  ...
  try {
    // Step 1: Token 발급 요청
    console.log('[SUGI_STEP1] Token Request for USER:', USERID);
    ...
    if (tokenData.code !== '0000') {
      throw new Error(`[TOKEN_ISSUE_FAILED] ${tokenData.msg}`);
    }

    const { ReturnURL, TOKEN } = tokenData;
    console.log('[SUGI_STEP1_SUCCESS] Token issued, proceeding to Pay URL:', ReturnURL);
    ...
    const payRes = await fetch(ReturnURL, {
      ...
      body: JSON.stringify({
        ...
        card_useable: cardExpiry.replace(/\//g, ''), // MM/YY -> MMYY
    ...
    if (payResult.code === '0000') {
      ...
      const msg = `
💳 <b>[수기결제 성공] lovelypetsitter.com</b>
...
🧾 승인번호: <code>${payResult.a_number}</code>
    ...
    console.error('[SUGI_FATAL_ERROR]', error);
